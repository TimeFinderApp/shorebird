...
class ShorebirdEnv {
  ...
  /// Get the Flutter version from FVM if available
  String? getFvmVersion() {
    final fvmrcFile = File(
      p.join(getFlutterProjectRoot()?.path ?? '', '.fvmrc'),
    );
    if (!fvmrcFile.existsSync()) return null;
